  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right", "bottom"]}>
      <View style={styles.page}>
        ...
        {syncDevices.length ? <View accessibilityLabel="同步设备状态" style={styles.syncDots}>{syncDevices.map((device) => <View accessibilityLabel={`${device.name}：${isSyncDeviceOnline(device) ? "在线" : "离线"}`} accessibilityRole="image" key={device.id} style={[styles.syncDot, isSyncDeviceOnline(device) ? styles.syncDotOnline : styles.syncDotOffline]} />)}</View> : null}
