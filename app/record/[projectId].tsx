...
export default function RecordingScreen() {
  ...
  const syncExitStateRef = useRef({ active: false, sentenceIndex: 0 });
  ...
  }, [activeSyncProjectKey, currentIndex, reportState, syncStatus.mode, syncStatus.projectId]);
  useEffect(() => {
  ...
  useEffect(() => () => {
    const exitingSync = syncExitStateRef.current;
    if (exitingSync.active) reportStateRef.current({ state: "idle", sentenceIndex: exitingSync.sentenceIndex });
...
const responsive = StyleSheet.create({
  headerCenter: { flex: 1, marginHorizontal: 8, minWidth: 0 },
