...
export default function ReplaceScriptScreen() {
  ...
  return <ScreenContainer className="px-5" edges={["top", "left", "right", "bottom"]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}><View style={styles.header}><TouchableOpacity style={[styles.back, glass.back]} onPress={() => router.back()}><MaterialIcons color="#4669DE" name="arrow-back" size={23} /></TouchableOpacity><Text style={[styles.title, glass.title]}>{copy.title}</Text><View style={styles.back} /></View><View style={[styles.notice, glass.notice]}><MaterialIcons color="#4669DE" name="verified-user" size={20} /><Text style={[styles.noticeText, glass.noticeText]}>{copy.notice}</Text></View><TouchableOpacity style={[styles.importBox, glass.importBox]} onPress={pickFiles} disabled={importing}>{importing ? <ActivityIndicator color="#4669DE" /> : <><View style={[styles.fileIcon, glass.fileIcon]}><MaterialIcons color="#4669DE" name="upload-file" size={27} /></View><View style={styles.importCopy}><Text style={[styles.importTitle, glass.importTitle]}>{fileName || copy.choose}</Text><Text style={[styles.importHint, glass.importHint]}>{sentences.length ? copy.parsed(sentences.length) : copy.line}</Text></View><MaterialIcons color="#8292B1" name="chevron-right" size={24} /></>}</TouchableOpacity><TouchableOpacity style={[styles.directoryButton, glass.directoryButton]} onPress={() => router.push(`/file-transfer?projectId=${project.id}` as never)} disabled={importing}><MaterialIcons color="#4669DE" name="wifi" size={20} /><Text style={[styles.directoryText, glass.directoryText]}>{t("fileTransfer")}</Text></TouchableOpacity>{sentences.length > 0 && <GlassSurface style={[styles.preview, glass.preview]} intensity={26}><Text style={[styles.previewTitle, glass.previewTitle]}>{copy.preview}</Text><Text style={[styles.previewText, glass.previewText]}>{sentences.slice(0, 4).map((sentence) => `${sentence.index}. ${sentence.rawText}${sentence.prompt ? ` (${sentence.prompt})` : ""}`).join("\n")}</Text></GlassSurface>}<TouchableOpacity style={[styles.confirm, glass.confirm, !sentences.length && styles.confirmDisabled]} onPress={confirm} disabled={!sentences.length}><Text style={styles.confirmText}>{copy.confirm}</Text></TouchableOpacity></ScrollView></ScreenContainer>;
}
...
const glass = StyleSheet.create({
  back: { backgroundColor: "rgba(255,255,255,0.58)", borderColor: "rgba(255,255,255,0.88)", borderRadius: 13, borderWidth: 1, elevation: 2, shadowColor: "#5977AE", shadowOpacity: 0.12, shadowRadius: 10 }, title: { color: "#182B55" },
  notice: { backgroundColor: "rgba(225,235,255,0.68)", borderColor: "rgba(255,255,255,0.86)", borderWidth: 1 }, noticeText: { color: "#415A84" }, importBox: { backgroundColor: "rgba(255,255,255,0.5)", borderColor: "rgba(112,143,221,0.52)", elevation: 2, shadowColor: "#6380B7", shadowOpacity: 0.08, shadowRadius: 12 }, fileIcon: { backgroundColor: "rgba(224,235,255,0.86)" }, importTitle: { color: "#182B55" }, importHint: { color: "#61749E" }, directoryButton: { backgroundColor: "rgba(225,235,255,0.7)", borderColor: "rgba(255,255,255,0.86)", borderWidth: 1 }, directoryText: { color: "#4669DE" }, preview: { borderRadius: 18, marginTop: 14, padding: 14 }, previewTitle: { color: "#3B527B" }, previewText: { color: "#61749E" }, confirm: { backgroundColor: "#4B6FE6", borderColor: "rgba(255,255,255,0.78)", borderWidth: 1, shadowColor: "#3D5FAE", shadowOpacity: 0.24, shadowRadius: 16 },
});
