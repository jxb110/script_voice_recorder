  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right", "bottom"]}>
      <View style={styles.header}>
        <TouchableOpacity style={[styles.back, glass.back]} onPress={goBack} accessibilityLabel={copy.back}><MaterialIcons color="#4669DE" name="arrow-back" size={24} /></TouchableOpacity>
        <View style={[styles.titleBlock, compact.titleBlock]}><Text numberOfLines={1} style={[styles.title, glass.title]}>{copy.title}</Text><Text numberOfLines={2} style={[styles.subtitle, glass.subtitle]}>{copy.subtitle}</Text></View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <View style={[styles.speakerCard, glass.speakerCard]}><TouchableOpacity style={[styles.speakerMain, compact.speakerMain]} onPress={() => openEdit(item)}><View style={[styles.avatar, glass.avatar]}><Text style={styles.avatarText}>{item.name.slice(0, 1)}</Text></View><View style={[styles.speakerBody, compact.minWidth]}><Text numberOfLines={1} style={[styles.name, glass.name]}>{item.name}</Text><Text numberOfLines={1} style={[styles.meta, glass.meta]}>{genderLabel(item.gender)} · {item.age} {copy.years}</Text></View><MaterialIcons color="#8293B4" name="edit" size={20} /></TouchableOpacity><TouchableOpacity style={[styles.delete, glass.delete]} onPress={() => confirmDelete(item)} accessibilityLabel={`${copy.delete} ${item.name}`}><MaterialIcons color="#C34F5A" name="delete-outline" size={20} /></TouchableOpacity></View>}
      ...
      <Modal visible={isCreating || Boolean(editing)} animationType="slide" transparent onRequestClose={close}>
        <View style={styles.overlay}><GlassSurface style={[styles.sheet, glass.sheet]} intensity={42}>
          <View style={styles.sheetHeader}><TouchableOpacity style={[styles.sheetBack, glass.sheetBack]} onPress={close} accessibilityLabel={copy.listBack}><MaterialIcons color="#4669DE" name="arrow-back" size={23} /></TouchableOpacity><Text numberOfLines={1} style={[styles.sheetTitle, glass.sheetTitle, compact.sheetTitle]}>{editing ? copy.edit : copy.create}</Text><TouchableOpacity style={[styles.sheetBack, glass.sheetBack]} onPress={close} accessibilityLabel={copy.close}><MaterialIcons color="#61749E" name="close" size={23} /></TouchableOpacity></View>
          <Text style={[styles.label, glass.label]}>{copy.name}</Text><TextInput autoFocus value={draft.name} onChangeText={(name) => setDraft((current) => ({ ...current, name }))} placeholder={copy.namePlaceholder} placeholderTextColor="#8493AF" style={[styles.input, glass.input]} returnKeyType="done" />
          <Text style={[styles.label, glass.label]}>{copy.gender}</Text><View style={styles.genders}>{(["女", "男", "其他"] as Gender[]).map((gender) => <TouchableOpacity key={gender} style={[styles.gender, glass.gender, draft.gender === gender && styles.genderActive]} onPress={() => setDraft((current) => ({ ...current, gender }))}><Text style={[styles.genderText, glass.genderText, draft.gender === gender && styles.genderTextActive]}>{genderLabel(gender)}</Text></TouchableOpacity>)}</View>
          <Text style={[styles.label, glass.label]}>{copy.age}</Text><TextInput value={String(draft.age)} onChangeText={(age) => setDraft((current) => ({ ...current, age: Number(age.replace(/[^0-9]/g, "")) || 0 }))} keyboardType="number-pad" style={[styles.input, glass.input]} />
          <TouchableOpacity style={[styles.save, glass.save]} onPress={save}><Text style={styles.saveText}>{copy.save}</Text></TouchableOpacity>
...
const compact = StyleSheet.create({
  titleBlock: { minWidth: 0, paddingHorizontal: 8 },
  speakerMain: { minWidth: 0 },
  minWidth: { minWidth: 0 },
  sheetTitle: { flex: 1, marginHorizontal: 10, textAlign: "center" },
});

const glass = StyleSheet.create({
  back: { backgroundColor: "rgba(255,255,255,0.58)", borderColor: "rgba(255,255,255,0.88)", borderRadius: 14, borderWidth: 1, elevation: 2, shadowColor: "#5977AE", shadowOpacity: 0.12, shadowRadius: 10 }, title: { color: "#182B55" }, subtitle: { color: "#61749E" },
  speakerCard: { backgroundColor: "rgba(255,255,255,0.54)", borderColor: "rgba(255,255,255,0.86)", elevation: 2, shadowColor: "#6380B7", shadowOpacity: 0.09, shadowRadius: 12 }, avatar: { backgroundColor: "rgba(236,233,255,0.86)" }, name: { color: "#182B55" }, meta: { color: "#61749E" }, delete: { borderLeftColor: "rgba(112,137,182,0.17)" },
  sheet: { backgroundColor: "rgba(247,251,255,0.82)", borderColor: "rgba(255,255,255,0.94)", borderWidth: 1 }, sheetBack: { backgroundColor: "rgba(225,235,255,0.72)", borderColor: "rgba(255,255,255,0.88)", borderRadius: 11, borderWidth: 1 }, sheetTitle: { color: "#182B55" }, label: { color: "#3B527B" }, input: { backgroundColor: "rgba(255,255,255,0.64)", borderColor: "rgba(255,255,255,0.88)", borderWidth: 1, color: "#182B55" }, gender: { backgroundColor: "rgba(232,239,250,0.68)", borderColor: "rgba(255,255,255,0.72)", borderWidth: 1 }, genderText: { color: "#61749E" }, save: { backgroundColor: "#4B6FE6", borderColor: "rgba(255,255,255,0.78)", borderWidth: 1, shadowColor: "#3D5FAE", shadowOpacity: 0.23, shadowRadius: 16 },
});
