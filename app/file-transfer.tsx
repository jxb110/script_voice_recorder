...
export default function FileTransferScreen() {
  ...
  return <ScreenContainer className="px-5" edges={["top", "left", "right", "bottom"]}>
    <View style={styles.page}>
      <View style={styles.header}><TouchableOpacity style={[styles.headerButton, glass.headerButton]} onPress={() => router.back()}><MaterialIcons color="#4669DE" name="arrow-back" size={23} /></TouchableOpacity><Text numberOfLines={1} style={[styles.title, glass.title, compact.title]}>{t("fileTransfer")}</Text><View style={styles.headerButton} /></View>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <GlassSurface style={[styles.hero, glass.hero]} intensity={34}><View style={styles.heroIcon}><MaterialIcons color="#FFFFFF" name="wifi" size={30} /></View><Text style={[styles.heroTitle, glass.heroTitle, compact.heroTitle]}>{status.running ? copy.running : copy.ready}</Text><Text style={[styles.heroText, glass.heroText]}>{t("fileTransferHint")}</Text></GlassSurface>
...
const compact = StyleSheet.create({
  title: { flex: 1, marginHorizontal: 10, textAlign: "center" },
  heroTitle: { textAlign: "center" },
  minWidth: { minWidth: 0 },
  tokenText: { flex: 1, flexShrink: 1, lineHeight: 18 },
});
