...
export default function FileTransferScreen() {
  ...
  const copy = language === "zh" ? {
    running: "局域网文件快传已开启", ready: "准备开启文件快传", opening: "正在确认端口监听…", address: "在同一 Wi-Fi 的设备浏览器中打开", token: "读写口令", notice: "快传仅在本页开启且手机应用保持前台时有效。地址含有读写口令，请勿发送给不受信任的人。", received: "已接收脚本", receivedHint: "在其他设备上传后，选择一个脚本继续。", use: "使用", empty: "暂未接收 TXT 脚本。请在其他设备浏览器打开上方地址上传。", stop: "停止文件快传", startFailed: "无法启动文件快传。", errorTitle: "文件快传未开始监听", retry: "重新监听端口",
  } : {
    running: "Local file transfer is running", ready: "Ready to start file transfer", opening: "Confirming the listening port…", address: "Open this in a browser on the same Wi-Fi", token: "Read/write token", notice: "Transfer works only while this page is open and the app stays in the foreground. The address includes a read/write token; do not share it with untrusted devices.", received: "Received scripts", receivedHint: "After uploading on another device, choose one script to continue.", use: "Use", empty: "No TXT script received yet. Open the address above in another device browser to upload.", stop: "Stop file transfer", startFailed: "Unable to start file transfer.", errorTitle: "File transfer is not listening", retry: "Retry port",
  };
