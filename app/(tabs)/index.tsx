  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <View style={styles.header}>
        <View>
          <Text style={styles.eyebrow}>本地语音采集</Text>
          <Text style={styles.title}>采音脚本</Text>
          <Text style={styles.subtitle}>导入文本，逐句完成高质量录音</Text>
        </View>
      ...
      <View style={styles.stats}>
        <View style={styles.stat}><Text style={styles.statNumber}>{projects.length}</Text><Text style={styles.statLabel}>录音任务</Text></View>
        <View style={styles.statDivider} />
        <View style={styles.stat}><Text style={styles.statNumber}>{recordedSentences}<Text style={styles.statTotal}>/{totalSentences}</Text></Text><Text style={styles.statLabel}>已录句子</Text></View>
        <View style={styles.statDivider} />
        <View style={styles.stat}><Text style={styles.statNumber}>{speakers.length}</Text><Text style={styles.statLabel}>发音人</Text></View>
      </View>
      <TouchableOpacity style={styles.primary} onPress={() => router.push("/new-project" as never)}>
        ...
        <View style={styles.primaryCopy}><Text style={styles.primaryTitle}>新建录音任务</Text><Text style={styles.primaryHint}>选择发音人并导入文本脚本</Text></View>
        <MaterialIcons color="#FFFFFF" name="arrow-forward" size={21} />
      </TouchableOpacity>
      <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>最近任务</Text><TouchableOpacity onPress={() => router.push("/(tabs)/scripts" as never)}><Text style={styles.viewAll}>全部任务</Text></TouchableOpacity></View>
      <FlatList
